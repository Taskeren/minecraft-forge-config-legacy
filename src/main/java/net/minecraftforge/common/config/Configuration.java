...
import static net.minecraftforge.common.config.ForgeConfigConst.JAVA_LETTER_OR_DIGIT;
...
public class Configuration {
	...
	public static final CharMatcher allowedProperties = JAVA_LETTER_OR_DIGIT.or(CharMatcher.anyOf(ALLOWED_CHARS));
	...
	public static File appHome = new File(".");
	public static File configDir = new File(appHome, "config");
	...
	public Configuration(File file, String configVersion) {
		...
		String basePath = appHome.getAbsolutePath().replace(File.separatorChar, '/').replace("/.", "");
		...
		if(PARENT != null) {
			...
			try {
				...
				logger.error("An exception occurred while loading config file {}. This file will be renamed to {} " +
	...
	public static void enableGlobalConfig() {
		PARENT = new Configuration(new File(configDir, "global.cfg"));
		PARENT.load();
	}
	...
	public int getInt(String name, String category, int defaultValue, int minValue, int maxValue, String comment, String langKey) {
		...
		prop.setMinValue(minValue);
		prop.setMaxValue(maxValue);
		return Math.max(minValue, Math.min(maxValue, prop.getInt(defaultValue)));
	}
	...
	public float getFloat(String name, String category, float defaultValue, float minValue, float maxValue, String comment, String langKey) {
		...
		prop.setMaxValue(maxValue);
		try {
			return Math.max(minValue, Math.min(maxValue, Float.parseFloat(prop.getString())));
		} catch(Exception e) {
